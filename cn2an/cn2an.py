...
class Cn2An():
    ...
    def cn2an(self, input_data=u"default_key"):
        ...
        if input_data:
            ...
            # 切割整数部分和小数部分
            split_result = input_data.split(u"点")
    ...
    @staticmethod
    def input_from_terminal(input_data):
        len_argv = len(sys.argv)
        if len_argv == 1:
            if input_data == u"default_key":
                raise Exception(u"请在cn2an后输入需要转化的中文数字！")
            else:
                return input_data
        elif len_argv == 2:
            input_cn = sys.argv[1]
        else:
            raise Exception(u"cn2an后的参数过多！")

        return input_cn

    def check_input_data_is_valid(self, check_data):
        # 检查输入数据是否在规定的字典中
        all_check_keys = list(self.conf[u"number_unit"].keys())
        all_check_keys.append(u"点")
    ...
    def integer_convert(self, integer_data):
        ...
        for index in range(len(integer_data)-1, -1, -1):
            unit_key = self.conf[u"number_unit"].get(integer_data[index])
    ...
    def decimal_convert(self, decimal_data):
        ...
        for index in range(len(decimal_data)-1, -1, -1):
            unit_key = self.conf[u"number_unit"].get(decimal_data[index])
